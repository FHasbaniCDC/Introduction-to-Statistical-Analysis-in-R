...
sample(c('Joshua','Carl','Jack','Lawrence','Davis','Mya','Kim','Cathy','Samantha','James'), size=1)
...
#seed is set for reproducibility as the random number generator picks a different seed each time unless specified
...
#------------------------------------------#
set.seed(999) 
idx1 = sample(1:nrow(pd), round(nrow(pd) * 0.6)) #60%
...
set.seed(999)
library(rsample)
idx3 = initial_split(pd, prop = 0.8)  #creates an object to further use for training and testing
